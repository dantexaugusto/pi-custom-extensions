		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			try {
				if (params.chain && params.chain.length > 0) {
					...
					// Pre-populate the chain with pending results for the widget
					const allChainResults: SingleResult[] = params.chain.map((step, idx) => ({
						agent: step.agent,
						agentSource: "unknown",
						task: step.task,
						exitCode: -2, // pending
					...
					updateSubagentsWidget(ctx, "chain", allChainResults);

					for (let i = 0; i < params.chain.length; i++) {
						const step = params.chain[i];
						const taskWithContext = step.task.replace(/\{previous\}/g, previousOutput);

						// Mark current step as running in the widget
						allChainResults[i].exitCode = -1;
						updateSubagentsWidget(ctx, "chain", allChainResults);
						...
						const chainUpdate: OnUpdateCallback | undefined = onUpdate
							? (partial) => {
									...
									if (currentResult) {
										allChainResults[i] = currentResult;
										updateSubagentsWidget(ctx, "chain", allChainResults);
				...
				if (params.tasks && params.tasks.length > 0) {
					...
					updateSubagentsWidget(ctx, "parallel", allResults);
				...
				if (params.agent && params.task) {
					...
					updateSubagentsWidget(ctx, "single", initialResults);
					...
					const result = await runSingleAgent(
						...
						(partial) => {
							if (partial.details?.results[0]) {
								updateSubagentsWidget(ctx, "single", [partial.details.results[0]]);
							}
							onUpdate?.(partial);
						},
						makeDetails("single"),
					);
					updateSubagentsWidget(ctx, "single", [result]);
				...
				// Clear the widget so it doesn't stay visible after execution finishes
				if (ctx.hasUI) {
					ctx.ui.setWidget("subagents-tracker", undefined);
				}
			}
		},
